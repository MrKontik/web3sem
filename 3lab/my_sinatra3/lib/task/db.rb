...
namespace :db do
  ...
  ENV['DATABASE'] = 'api_hh_development' if environment == 'development'
  ENV['DATABASE'] = 'api_hh_test' if environment == 'test'
  ENV['DATABASE'] = 'api_hh_production' if environment == 'production'
  # connection_string = ENV['DATABASE_URL'] ||
  ENV["DATABASE_URL_#{environment.upcase}"]
  migrations_directory = 'db/migrations'
  connection_string =
    "postgres://postgres:1234@localhost/#{ENV['DATABASE']}"
